...
@section('content')
    <div class="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="rounded-tl-[2rem] rounded-tr-[2rem] rounded-br-[2rem] rounded-bl-md border p-8 sm:p-10" style="background: color-mix(in srgb, var(--theme-surface) 94%, transparent); border-color: var(--theme-border); box-shadow: var(--theme-glow);">
            <div class="flex flex-col items-center gap-6 text-center sm:flex-row sm:text-left">
                ...
                <div>
                    ...
                    <h1 class="mt-2 flex items-center justify-center gap-3 font-bold font-[Fredoka] text-5xl sm:justify-start">
                        {{ $profileUser->username }}
                        @if ($profileUser->is_master_admin)
                            <span class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold" style="background: color-mix(in srgb, var(--theme-accent) 18%, transparent); color: var(--theme-accent);" title="Master Admin">
                                <x-icon name="crown" class="h-3.5 w-3.5" /> Master Admin
                            </span>
                        @endif
                    </h1>
                    @if ($profileUser->birthday)
                        <p class="mt-2 flex items-center justify-center gap-1.5 text-sm sm:justify-start" style="color: var(--theme-muted);"><x-icon name="calendar" class="h-4 w-4" /> {{ $profileUser->birthday->format('F j, Y') }}</p>
                    @endif
